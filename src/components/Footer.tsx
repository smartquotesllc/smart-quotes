...
export function Footer() {
  return (
    <footer className="bg-sq-black text-white">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[1.2fr_1fr_1fr] lg:px-8">
        ...
        <div>
          ...
          <ul className="space-y-3">
            {LINKS.map((link) => (
              <li key={link.href}><Link href={link.href} className="text-sm text-white/80 transition hover:text-white">{link.label}</Link></li>
            ))}
          </ul>
        </div>
        <div>
          <h2 className="mb-4 font-[family-name:var(--font-montserrat)] text-xs font-bold uppercase tracking-[0.16em] text-white/50">Legal</h2>
          <ul className="space-y-3">
            {LEGAL.map((link) => (
              <li key={link.href}><Link href={link.href} className="text-sm text-white/80 transition hover:text-white">{link.label}</Link></li>
            ))}
          ...
          <div className="mt-8 flex gap-3" aria-label="Social links">
            {["Instagram", "Facebook", "X", "LinkedIn"].map((name) => (
              <span key={name} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/20 text-[10px] font-semibold uppercase tracking-wide text-white/70" title={`${name} (placeholder)`}>{name.slice(0, 2)}</span>
            ))}
          </div>
        </div>
      </div>
